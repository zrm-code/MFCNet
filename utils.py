...
class WeightedBCE(nn.Module):
    ...
    def forward(self, logit_pixel, truth_pixel):
        ...
        # print("WeightedBCE",logit_pixel.shape,truth_pixel.shape,)
...
class WeightedDiceBCE(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        # return 0.6 * dice + 0.4 * BCE  # SwinUnet
        return dice_BCE_loss # TransUNet SwinUnet DSTransNet MSESNet UCTransNet MT-UNet
        # return BCE  # multiResUNet AttenUNet CANet MedT
        # return dice  #DCSAU-Net
        # return 0.5 * IoU + 0.5 * BCE  DSTransNet
...
# the index of mACC，计算分割二分类的的分割准度。
def acc_on_batch(masks, pred):
    ...
    for i in range(pred.shape[0]):
        ...
        # print("rrr",np.max(mask), np.min(mask))
        acc.append(accuracy_score(mask_tmp.reshape(-1), pred_tmp.reshape(-1)))
    return np.mean(acc)
